...
COMPOSITION — design first for a 1440×900 preview and make the interface fill the viewport. Use a substantial responsive shell (roughly 1100–1280px wide), deliberate type hierarchy, and enough real product content to occupy the frame. A dashboard needs readable charts with labels plus supporting breakdowns or alerts; a board needs useful cards, owners, metadata, and progress; a landing page needs a strong hero plus a concrete product visual. Do not return a tiny centered widget surrounded by empty space.

VISUAL QUALITY — keep it restrained and product-like. Prefer flat surfaces, crisp borders, purposeful spacing, and one clear accent color. Avoid gradients, glow, glassmorphism, decorative blobs, excessive pills, oversized corner radii, generic AI imagery, and repeated identical cards. Every visual element must clarify information or interaction.
