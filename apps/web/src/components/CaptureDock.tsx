...
export function CaptureDock({
  ...
  if (!hostMode) {
    return (
      <div className="watchDock" aria-label="Viewer controls">
        <div className="watchRoom">
          <span className={"watch-dot" + (state.capture.screen ? " on" : "")} aria-hidden="true" />
          <span className="watchHost">{state.capture.host ?? "Host"}&rsquo;s room</span>
        </div>
        <Button variant="ghost" size="sm" className="watchAction" asChild>
          <a href={withHostParam()}>
            <UserRoundCog aria-hidden="true" />
            Host view
          </a>
        </Button>
        <Button variant="ghost" size="sm" className="watchAction watchExit" onClick={onLeave} data-tip="Leave the meeting">
  ...
  return (
    <div className="captureDock">
      ...
      <Button size="sm" className="capBtn primary" onClick={startRoom} data-tip="Start the live room so others can join">
        Go live
      </Button>
      <Button
        variant="outline"
      ...
      />
      <Button variant="outline" size="sm" className="capBtn" onClick={sendManual} data-tip="Send a typed line into the transcript">
        Send
      </Button>
      <Button
        variant="destructive"
      ...
      >
        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-circle-stop"><circle cx="12" cy="12" r="10"/><rect x="9" y="9" width="6" height="6" rx="1"/></svg>
        End meeting
      </Button>
      <Button
        variant="outline"
